...
bool Box_GUI::checkCollision(glm::vec2 const& r2p, glm::vec2 const& r2s)
{
    return _checkCollision({ _pos.x, _pos.y - _size.y }, _size, r2p, r2s);
}

bool Box_GUI::checkCollision(std::shared_ptr<SSS::GL::PlaneBase> plane)
{
...
glm::vec3 Box_GUI::center() const
{
    return _pos + glm::vec3(_size.x / 2.f, _size.y / -2.f, 0);
}

void Box_GUI::setPos(glm::vec3 pos)
{
    _pos = pos;
...
void Box_GUI::setColor(glm::vec4 color)
{
    _color = color;
    for (auto& plane : model) {
...
void Box_GUI::set_text_data(const Text_data& td)
{
    _td = td;
    create_box();
}
...
void Box_GUI::create_box()
{
    ...
    // Create the model
    _create_part(_td.text_ID, Box_GUI::layout_map["ID"], FLAG_ID);
    // Text
    _create_part(_td.text, Box_GUI::layout_map["TEXT"], FLAG_TEXT);
    // Comment text
    if (!_td.comment.empty())
        _create_part(_td.comment, Box_GUI::layout_map["COMMENT"], FLAG_TEXT);
    // tags
